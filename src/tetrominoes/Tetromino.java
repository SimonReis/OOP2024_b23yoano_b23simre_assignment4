...
public class Tetromino {
	 ...
	 */
	public Tetromino(TetrisGame tetrisGame) {
	...
	public void moveDown() {
		...
		if (canMoveDown()) {
			...
			for (int rowGrid = tetrisGame.getFrame().getGameGrid().getRowCount() - 1; rowGrid >= 0; rowGrid--) {
				for (int colGrid = 0; colGrid < tetrisGame.getFrame().getGameGrid()
						.getColumnCount(); colGrid++) {
					if (tetrisGame.getFrame().getGameGrid().getValueAt(rowGrid,
							colGrid) == tetrominoShape) {
						tetrisGame.getFrame().getGameGrid().setValueAt(null, rowGrid, colGrid);
						tetrisGame.getFrame().getGameGrid().setValueAt(tetrominoShape, rowGrid + 1,
								colGrid);
					}
				}
			}
			...
			for (int rowGrid = 0; rowGrid < tetrisGame.getFrame().getGameGrid()
					.getRowCount(); rowGrid++) {
				for (int colGrid = 0; colGrid < tetrisGame.getFrame().getGameGrid()
						.getColumnCount(); colGrid++) {
					if (tetrisGame.getFrame().getGameGrid().getValueAt(rowGrid,
							colGrid) == tetrominoShape) {
						tetrisGame.getFrame().getGameGrid().setValueAt(tetrominoShapeDuo[1], rowGrid,
								colGrid);
					}
				}
			}
			...
			for (int rowGrid = 0; rowGrid < tetrisGame.getFrame().getGameGrid()
				...
				for (int colGrid = 0; colGrid < tetrisGame.getFrame().getGameGrid()
					...
					if (tetrisGame.getFrame().getGameGrid().getValueAt(rowGrid,
							colGrid) == tetrominoShape) {
						tetrisGame.getFrame().getGameGrid().setValueAt(null, rowGrid, colGrid);
						tetrisGame.getFrame().getGameGrid().setValueAt(tetrominoShape, rowGrid,
								colGrid - 1);
					}
				}
			}
	...
	public void moveRight() {
		...
		if (canMoveRight()) {
			...
			for (int rowGrid = 0; rowGrid < tetrisGame.getFrame().getGameGrid()
					.getRowCount(); rowGrid++) {
				for (int colGrid = tetrisGame.getFrame().getGameGrid().getColumnCount()
						- 1; colGrid >= 0; colGrid--) {
					if (tetrisGame.getFrame().getGameGrid().getValueAt(rowGrid,
							colGrid) == tetrominoShape) {
						tetrisGame.getFrame().getGameGrid().setValueAt(null, rowGrid, colGrid);
	...
	public boolean isSpawnable() {
		...
		try {
			for (int rowMatrix = 0; rowMatrix < matrix.length; rowMatrix++) {
				for (int colMatrix = 0; colMatrix < matrix[0].length; colMatrix++) {
					if (matrix[rowMatrix][colMatrix] == 1) {
						if (TetrisGame.getGameInstance().getFrame().getGameGrid()
								.getValueAt(row + rowMatrix - offsetTop, col + colMatrix) != null) {
							return false;
						}
					}
				}
